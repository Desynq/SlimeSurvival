...
public class MystiteArmor extends ArmorItem {
	...
	public MystiteArmor(EquipmentSlot slot, Settings settings) {
		super(SSArmorMaterials.MYSTITE, slot, settings);

		this.type = SSArmorMaterials.MYSTITE;
		this.slot = slot;
		this.protection = this.type.getProtectionAmount(slot);
		this.toughness = this.type.getToughness();
		this.knockbackResistance = this.type.getKnockbackResistance();
